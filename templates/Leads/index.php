                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-12 mt-3">
                            <form action="<?php echo $this->Url->build(['controller' => 'leads', 'action' => 'pesquisa']); ?>" class="service-search" method="GET">
                                <div class="row">
                                    <div class="col-md-8">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <input name="s" type="text" placeholder="Pesquisar Lead" class="service-input form-control" value="">
                                            </div>
                                            <div class="col-md-6">
                                    ...
                                    <div class="col-md-4">
                                        <?= $this->Html->link(__('Adicionar Lead'), ['action' => 'add'], ['class' => 'btn btn-pill float-right mx-1 btn-primary']) ?>
                                    </div>
                                </div>
                            </form>
                        </div>
...
foreach ($situations as $situationSingle) {
    $listTech[] = array(
        'id' => $situationSingle->id,
        'title' => $situationSingle->title
    );
}
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventClick: function(info) {
                window.location.href = '<?php echo $this->Url->build(['controller' => 'leads', 'action' => 'view']); ?>/' + info.event.id;
            },
            eventDrop: function(info) {
                ...
                $.post('<?php echo $this->Url->build(['controller' => 'get', 'action' => 'editlead']); ?>', {
                    id: id,
                    date: newData,
                    technician_id
                }).done();
            ...
            events: {
                url: '<?php echo $this->Url->build(['controller' => 'get', 'action' => 'leads']); ?>',
                failure: function() {
