...
class GetController extends AppController
{
    ...
    public function services()
    {
        ...
        $pesquisaPermission = ($this->usuarioAtual['role_id'] == 1 || $this->usuarioAtual['role_id'] == 2) ? ['date >= ' => $start, 'date <= ' => $end]: ['date >= ' => $start, 'date <= ' => $end, 'Services.seller_id' => $this->usuarioAtual['id']];
        $services = $servicesTable->find('all')->contain(['Clients'])->where($pesquisaPermission);
        $this->set(compact('services'));
